...
namespace QuanLyBanHangSieuThi.DAL
{
    public class SellerDAL
    {
        ...
        private string _dataFile = Path.Combine(Global.DATA_PATH, nameof(Seller) + ".json");
        ...
        // Phương thức nạp danh sách Seller từ file JSON
        public List<Seller> Load()
        {
            if (File.Exists(_dataFile))
            {
                try
                {
                    ...
                    return JsonSerializer.Deserialize<List<Seller>>(jsonString, options);
                }
                catch (Exception ex)
                {
            ...
            return new List<Seller>(); // Trả về danh sách trống nếu không có file
        }

        // Phương thức lưu danh sách Seller vào file JSON
        public void Save(List<Seller> Sellers)
        {
            try
            {
                ...
                var jsonString = JsonSerializer.Serialize(Sellers, options);
